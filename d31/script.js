...
d3.csv(myfile, function (data) {
  ...
  var xScale = d3.scale.linear()
    .domain([
    	...
    	d3.max([85,d3.max(data,function (d) { return d.x })])
    	])
    .range([0,w])
  var yScale = d3.scale.linear()
    .domain([
    	d3.min([70,d3.min(data,function (d) { return d.y })]),
    	d3.max([120,d3.max(data,function (d) { return d.y })])
    	])
    .range([h,0])
	// SVG
  ...
  var circles = svg.selectAll('circle')
    ...
    .append('title') // Tooltip
      .text(function (d) { return d.Dzimums +
                           '\nMiera pulss: ' + (d.x) +
                           '\nVidējais pulss videospēļu laikā: ' + (d.y) })
  // X-axis
  svg.append('g')
      .attr('class','axis')

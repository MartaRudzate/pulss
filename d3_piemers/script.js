...
d3.csv(myfile, function (data) {
  ...
  var yScale = d3.scale.linear()
    .domain([
    	d3.min([40,d3.min(data,function (d) { return d.pulss_videospeles })]),
    	d3.max([100,d3.max(data,function (d) { return d.pulss_videospeles })])
    	])
    .range([h,0])
	// SVG
  ...
  var circles = svg.selectAll('circle')
    ...
    .append('title') // Tooltip
      .text(function (d) { return d.ID +
                           ...
                           '\nfiziskās sagatavotības indekss: ' + (d.pulss_videospeles) })
  // X-axis
  svg.append('g')
      .attr('class','axis')
